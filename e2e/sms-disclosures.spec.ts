...
test('SMS Program Disclosures page renders key compliance content at /sms-disclosures', async ({
  ...
  await expect(disclosures.getByRole('link', { name: /privacy policy/i })).toHaveAttribute(
    'href',
    '/privacy',
  );
  await expect(disclosures.getByRole('link', { name: /terms of service/i })).toHaveAttribute(
    'href',
    '/terms',
  );
});
